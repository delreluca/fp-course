...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f m = StateT (\s -> (\(a,t) -> (f a, t)) <$> runStateT m s)
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (<*>) m n = StateT (\s -> (\(f,t) -> (\(a,u) -> (f a, u)) <$> runStateT n t) =<< (runStateT m s))
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) g m = StateT (\s -> (\(a,t) -> runStateT (g a) t) =<< runStateT m s)
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' m s = case runStateT m s of
                     ExactlyOne (a,t) -> (a,t)
...
distinct' xs = eval (filtering p xs) S.empty
  where p x = State(\s -> case S.member x s of
                          True -> (False, s)
                          False -> (True, S.insert x s))
...
distinctF xs = evalT (filtering p xs) S.empty
  where p x = StateT(\s -> case x > 100 of
                                True -> Empty
                                False -> case S.member x s of
                                              True -> Full (False, s)
                                              False -> Full (True, S.insert x s))
...
instance Monad f => Applicative (OptionalT f) where
  ...
  (<*>) ft at = OptionalT ((\q -> case q of { Empty -> pure Empty; Full g -> ((g <$>) <$> a)}) =<< f)
    where f = runOptionalT ft
          a = runOptionalT at
          --OptionalT ((\q -> (\t -> t <*> q) <$> f) =<< a) is another way, but the other way round!! here if f=List the function list is unrolled first
...
instance Applicative (Logger l) where
  ...
  (<*>) (Logger ls f) (Logger ks a) = Logger (ls ++ ks) (f a)
...
distinctG xs = runOptionalT $ evalT (filtering p xs) S.empty
  where p x = StateT(\s -> OptionalT(case x > 100 of
                                          True -> log1 ("aborting > 100: " ++ (myShow x)) Empty
                                          False -> case x `rem` 2 of
                                                        0 -> log1 ("even number: " ++ (myShow x)) (q x s)
                                                        1 -> pure (q x s)))
        q x s = case S.member x s of
                     True -> Full (False, s)
                     False -> Full (True, S.insert x s)
        myShow = listh . show
