...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy pred = b =<< character
  where b k
          | pred k = valueParser k
          | otherwise = unexpectedCharParser k
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = sequence -- This seems too easy, will check back later
...
firstNameParser ::
  Parser Chars
firstNameParser = (\k -> (k:.) <$> list lower) =<< upper
...
surnameParser ::
  Parser Chars
surnameParser = (\k -> (k:.) <$> fiveOrMore) =<<upper
  where fiveOrMore = (\fs -> (fs++) <$> list lower) =<< thisMany 5 lower
...
smokerParser ::
  Parser Bool
smokerParser = yay ||| nay
  where yay = pure True <* is 'y'
        nay = pure False <* is 'n'
...
phoneParser ::
  Parser Chars
phoneParser = (\k -> (\ks -> pure (k:.ks) <* is '#') =<< phoneBodyParser) =<< digit
