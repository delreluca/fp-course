...
noneof ::
  Chars
  -> Parser Char
noneof xs = satisfy $ flip notElem xs
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok oc cc ap = between (charTok oc) (charTok cc) ap
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 ap sp = (\a -> (a:.) <$> list (sp *> ap)) =<< ap
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby ap sp = sepby1 ap sp ||| valueParser Nil
...
eof ::
  Parser ()
eof = P c
  where c Nil = Result Nil ()
        c s = UnexpectedString s
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma oc cc ap = betweenCharTok oc cc $ sepby ap $ commaTok
